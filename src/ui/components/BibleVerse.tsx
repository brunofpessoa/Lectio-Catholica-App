...
import { useTheme } from '../../presentation/ThemeState';
import { CicReference } from '../../domain/entities/bible/CicReference';
import { useBible } from '../../presentation/BibleState';
...
export function BibleVerse({ verse }: Props) {
  ...
  const getCicReferenceMultipleVerses = (
    cicReferences: CicReference[] | undefined,
  ) => {
    if (!cicReferences) {
      return [];
    }
  ...
  const formatCicReferenceMultipleVerses = (cr: CicReference) => {
    let chapters =
      cr.chapterStart !== cr.chapterEnd
        ? `${cr.chapterStart}-${cr.chapterEnd}`
        : String(cr.chapterStart);
    let verses =
      cr.verseStar !== cr.verseEnd
        ? `${cr.verseStar}-${cr.verseEnd}`
        : String(cr.verseStar);
  ...
  return (
    <View>
      <View style={styles.container}>
        <Text style={[colorScheme.text, styles.verseNumber]}>
          {verse.number}
        </Text>
        <Text style={[colorScheme.text, styles.verseText]}>{verse.text}</Text>
        <View style={styles.buttons}>
          {verse.cicReferences && <Button onPress={() => {}} title="CIC" />}
        </View>
      </View>
